...
int readbindata(void) {
    ...
    if (fread(data, 1, 5, ifile) != 5) return (0); /* EOF */
    v[0] = (data[0] & 0xff) | ((data[1] << 8) & 0xff00);
    v[1] = (data[2] & 0xff) | ((data[3] << 8) & 0xff00);
    if (data[4] == '.') return (1); /* samples, no annotation */
...
int readtxtdata(void) {
    ...
    if (c == '"') return (1); /* samples, no annotation */
    a.anntyp = ammap(c);
    a.subtyp = (c == 'U' ? -1 : 0);
    return (2); /* samples and annotation */
}
...
int main(int argc, char **argv) {
    char *p, *record;
    int i, sflag = 0, zflag = 0;

    if (argc < 2 || strcmp(argv[1], "-h") == 0) {
        fprintf(stderr, "usage: %s [-s] RECORD.XXX [RECORD.XXX]...\n", argv[0]);
        fprintf(stderr, " where RECORD.XXX is the name of the AHA DVD input file to"
                        " be converted\n"
                        ...
                        " Use -z to make short-format (30-minute) records from"
                        " long-format (2.5 hour)\n input files.  When using -z, the"
                        " second digit in the output record name is\n"
                        " incremented by 2.\n");
        exit(1);
    }
    if (strcmp(argv[i = 1], "-s") == 0) {
        ...
        zflag = 1; /* produce short-format (30-minute) records */
        i = 2;
    }
    for (; i < argc; i++) {
        ...
        *p = '\0';      /* strip off extension */
        ...
        } else if (zflag) { /* skip first 145 minutes if making a short-format record */
            if (format == 1)
                fseek(ifile, 150L * 60L * 250L * 5L, SEEK_SET);
            else if (format == 2) {
                long t;

                for (t = 0; t < 150L * 60L * 250L; t++)
                    (void)readtxtdata();
            }
            record[1] += 2; /* fix record name (n0nn->n2nn, n1nn->n3nn) */
        }
