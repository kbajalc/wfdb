    ...
    do {
        ...
        if (run_length[a] > 0) {
            ...
            case 'F':
                ...
                break;
            case ']':
                if (type == 0) /* This annotation might end the run. */
                    run_end = annot[a].time + match_dt;
                break;
            case 'S':
                if (type == 1) {
                ...
                break;
            case '}':
                if (type == 1) /* This annotation might end the run. */
                    run_end = annot[a].time + match_dt;
                break;
            default:
...
void pstat(char *s, long a, long b) {
    if (fflag != 2 && fflag != 5) {
        ...
        if (b <= 0)
            ...
            (void)fprintf(ofile, " %3d%%", (int)((100. * a) / b + 0.5));
...
int amap(WFDB_Annotation annot, unsigned int i) {
    ...
    case RHYTHM:
        if (annot.aux == NULL || *(annot.aux) == 0) return ('O');
...
/* initialize state variables for amap() */
void initamap() {
...
/* `find_longest_run' reads forward in annotation file `a' (the reference file
   ...
   type = 0: find VE run; type = 1: find SVE run
*/
int find_longest_run(unsigned int a, WFDB_Time t0, WFDB_Time t1, int type) {
    int am, len = 0, len0 = 0;
    ...
    while (annot[a].time < t0) {
        if ((type == 0 && amap(annot[a], a) == '[') || (type == 1 && amap(annot[a], a) == '{')) { /* VF (AF) begins */
            do {
                ...
                am = amap(annot[a], a);
            } while ((type == 0 && am != ']' && am != '{' && am != 'U') || (type == 1 && am != '}' && am != '[' && am != 'U'));
            if (annot[a].time > t0) {
    ...
    while (annot[a].time <= t1) {
        ...
        case 'F':
            if (type == 0) {
                /* These beats continue runs; we count up to 6 beats in each run. */
                if (len0 < 6) len0++;
            } else {
                if (len0 > len) len = len0;
                len0 = 0;
    ...
    for (i = 1; i < argc; i++) {
        if (*argv[i] == '-') switch (*(argv[i] + 1)) {
            ...
            case 't': /* end time follows */
                ...
                end_time = i;
                break;
            case 'v': /* verbose mode */ verbose = 1; break;
...
void print_results(int type) { /* 0: VE; 1: SVE */
    int i;
    ...
    if (type == 0) {
        if (strcmp(ofname, "-")) {
            if ((ofile = fopen(ofname, "r")) == NULL) {
                ...
                if (fflag == 2 || fflag == 5) {
                    (void)fprintf(ofile, "Record CTs CFN CTp CFP STs SFN STp SFP LTs LFN LTp LFP");
    ...
    if (type == 1 && fflag == 5) {
        if (strcmp(ofname, "-")) (void)fclose(ofile);
        if (strcmp(sfname, "-")) {
    ...
    if (fflag == 1 || fflag == 3 || fflag == 4 || fflag == 6) {
        (void)fprintf(ofile, "%sVE run-by-run comparison results for record %s\n", type ? "S" : "", record);
    ...
    CFP = s[1][0][2] + s[1][1][2];
    STPs = s[0][3][3] + s[0][3][4] + s[0][3][5] + s[0][3][6] + s[0][4][3] + s[0][4][4] + s[0][4][5] + s[0][4][6] + s[0][5][3] + s[0][5][4] +
           s[0][5][5] + s[0][5][6];
    SFN = s[0][3][0] + s[0][3][1] + s[0][3][2] + s[0][4][0] + s[0][4][1] + s[0][4][2] + s[0][5][0] + s[0][5][1] + s[0][5][2];
    STPp = s[1][3][3] + s[1][3][4] + s[1][3][5] + s[1][4][3] + s[1][4][4] + s[1][4][5] + s[1][5][3] + s[1][5][4] + s[1][5][5] + s[1][6][3] +
           s[1][6][4] + s[1][6][5];
    SFP = s[1][0][3] + s[1][0][4] + s[1][0][5] + s[1][1][3] + s[1][1][4] + s[1][1][5] + s[1][2][3] + s[1][2][4] + s[1][2][5];
    LTPs = s[0][6][6];
    LFN = s[0][6][0] + s[0][6][1] + s[0][6][2] + s[0][6][3] + s[0][6][4] + s[0][6][5];
    LTPp = s[1][6][6];
    LFP = s[1][0][6] + s[1][1][6] + s[1][2][6] + s[1][3][6] + s[1][4][6] + s[1][5][6];
    ...
    case 4:
        ...
        (void)fprintf(ofile, "  NC |      %4ld %4ld %4ld %4ld\n", s[0][0][1], s[0][0][2], s[0][0][3] + s[0][0][4] + s[0][0][5], s[0][0][6]);
        (void)fprintf(ofile, "   V | %4ld %4ld %4ld %4ld %4ld\n", s[0][1][0], s[0][1][1], s[0][1][2], s[0][1][3] + s[0][1][4] + s[0][1][5],
                      s[0][1][6]);
        (void)fprintf(ofile, "   C | %4ld %4ld %4ld %4ld %4ld\n", s[0][2][0], s[0][2][1], s[0][2][2], s[0][2][3] + s[0][2][4] + s[0][2][5],
                      s[0][2][6]);
        (void)fprintf(ofile, "   S | %4ld %4ld %4ld %4ld %4ld\n", s[0][3][0] + s[0][4][0] + s[0][5][0], s[0][3][1] + s[0][4][1] + s[0][5][1],
                      s[0][3][2] + s[0][4][2] + s[0][5][2],
                      s[0][3][3] + s[0][4][3] + s[0][5][3] + s[0][3][4] + s[0][4][4] + s[0][5][4] + s[0][3][5] + s[0][4][5] + s[0][5][5],
                      s[0][3][6] + s[0][4][6] + s[0][5][6]);
        (void)fprintf(ofile, "   L | %4ld %4ld %4ld %4ld %4ld\n", s[0][6][0], s[0][6][1], s[0][6][2], s[0][6][3] + s[0][6][4] + s[0][6][5],
                      s[0][6][6]);
        ...
        (void)fprintf(ofile, "   V | %4ld %4ld %4ld %4ld %4ld\n", s[1][1][0], s[1][1][1], s[1][1][2], s[1][1][3] + s[1][1][4] + s[1][1][5],
                      s[1][1][6]);
        (void)fprintf(ofile, "   C | %4ld %4ld %4ld %4ld %4ld\n", s[1][2][0], s[1][2][1], s[1][2][2], s[1][2][3] + s[1][2][4] + s[1][2][5],
                      s[1][2][6]);
        (void)fprintf(ofile, "   S | %4ld %4ld %4ld %4ld %4ld\n", s[1][3][0] + s[1][4][0] + s[1][5][0], s[1][3][1] + s[1][4][1] + s[1][5][1],
                      s[1][3][2] + s[1][4][2] + s[1][5][2],
                      s[1][3][3] + s[1][4][3] + s[1][5][3] + s[1][3][4] + s[1][4][4] + s[1][5][4] + s[1][3][5] + s[1][4][5] + s[1][5][5],
                      s[1][3][6] + s[1][4][6] + s[1][5][6]);
        (void)fprintf(ofile, "   L | %4ld %4ld %4ld %4ld %4ld\n", s[1][6][0], s[1][6][1], s[1][6][2], s[1][6][3] + s[1][6][4] + s[1][6][5],
                      s[1][6][6]);
        (void)fprintf(ofile, "\n (Run Positive Predictivity Summary Matrix)\n\n");
        break;
    case 2: /* print line-format output */
    case 5:
        (void)fprintf(ofile, "%4s %3ld %3ld %3ld %3ld %3ld %3ld %3ld %3ld %3ld %3ld %3ld %3ld ", record, CTPs, CFN, CTPp, CFP, STPs, SFN, STPp, SFP,
                      LTPs, LFN, LTPp, LFP);
        break;
    ...
    case 6:
        ...
        (void)fprintf(ofile, "   0 |      %4ld %4ld %4ld %4ld %4ld %4ld\n", s[1][0][1], s[1][0][2], s[1][0][3], s[1][0][4], s[1][0][5], s[1][0][6]);
        for (i = 1; i < 6; i++)
            (void)fprintf(ofile, "   %d | %4ld %4ld %4ld %4ld %4ld %4ld %4ld\n", i, s[1][i][0], s[1][i][1], s[1][i][2], s[1][i][3], s[1][i][4],
                          s[1][i][5], s[1][i][6]);
        (void)fprintf(ofile, "  >5 | %4ld %4ld %4ld %4ld %4ld %4ld %4ld\n", s[1][6][0], s[1][6][1], s[1][6][2], s[1][6][3], s[1][6][4], s[1][6][5],
                      s[1][6][6]);
        (void)fprintf(ofile, "\n (Run Positive Predictivity Summary Matrix)\n\n");
...
static char *help_strings[] = {"usage: %s -r RECORD -a REF TEST [OPTIONS ...]\n",
