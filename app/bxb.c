...
static int A, Aprime;                       /* types of the current & next reference annotations */
static int a, aprime;                       /* types of the current & next test annotations */
static int fflag = 3;                       /* report format (0: none; 1: compressed; 2: line;
                                       3: standard; 4: compressed with SVEB;  5: line
                                       with SVEB;  6: standard with SVEB) */
static int match_dt = 0;                    /* match window duration in samples */
static int Oflag = 0;                       /* if non-zero, produce an extended annotation file */
static WFDB_Time shut_down;                 /* duration of test annotator's shutdown */
...
static WFDB_Time T, Tprime;                 /* times of the current & next reference annotations */
static WFDB_Time t, tprime;                 /* times of the current & next test annotations */
...
static int amap(int a);
static int rpann(WFDB_Time t);
static int tpann(WFDB_Time t);
...
int bxb_main(int argc, char *argv[]) {
    ...
    /* If there is a test annotation within an interval equal to the match
       ...
       one. */
        ...
        gettest();
        if (t - start <= match_dt && abs(T - tprime) < abs(T - t)) gettest();
    }
    ...
    */
    while ((end_time > 0L && (T <= end_time || t <= end_time)) || (end_time == -1L && T != huge_time) || (end_time == 0L && T != huge_time && t != huge_time)) {
...
static char *record; /* record name */
static WFDB_Anninfo an[3];
static unsigned int oflag = 0;    /* if non-zero, produce an output annotation file */
static WFDB_Time RR;              /* reference RR interval, if non-zero */
static WFDB_Time sdonref = -1L;   /* start of reference shutdown */
static WFDB_Time sdoffref = -1L;  /* end of reference shutdown */
static WFDB_Time vfonref = -1L;   /* start of reference VF */
static WFDB_Time vfoffref = -1L;  /* end of reference VF */
static WFDB_Time psdonref = -1L;  /* start of previous reference shutdown */
static WFDB_Time psdoffref = -1L; /* end of previous reference shutdown */
static WFDB_Time pvfonref = -1L;  /* start of previous reference VF */
static WFDB_Time pvfoffref = -1L; /* end of previous reference VF */
static WFDB_Annotation ref_annot;

static void getref() /* get next reference beat annotation */
{
...
static WFDB_Time rr;               /* test RR interval, if non-zero */
static WFDB_Time sdontest = -1L;   /* start of test shutdown */
static WFDB_Time sdofftest = -1L;  /* end of test shutdown */
static WFDB_Time vfontest = -1L;   /* start of test VF */
static WFDB_Time vfofftest = -1L;  /* end of test VF */
static WFDB_Time psdontest = -1L;  /* start of previous test shutdown */
static WFDB_Time psdofftest = -1L; /* end of previous test shutdown */
static WFDB_Time pvfontest = -1L;  /* start of previous test VF */
static WFDB_Time pvfofftest = -1L; /* end of previous test VF */
static WFDB_Annotation test_annot;

static void gettest() /* get next test annotation */
{
            ...
            if ((annot.subtyp & 0x30) == 0x30) {
                ...
                if (getann(1, &annot) < 0) {
                    ...
                    if (end_time > 0L)
                        ...
                        (void)fprintf(stderr, "%s: unterminated shutdown starting at %s in record %s, annotator %s\n", pname, timstr(sdontest), record, an[1].name);
                        (void)fprintf(stderr, " (not included in shutdown duration measurement)\n");
                    }
                    return;
                }
                if (annot.anntyp == NOISE && (annot.subtyp & 0x30) != 0x30)
...
static int rpann(WFDB_Time t) {
    if ((vfonref != -1L && vfonref <= t && (t <= vfoffref || vfoffref == -1L)) || (pvfonref != -1L && pvfonref <= t && t <= pvfoffref))
    ...
    else if ((sdonref != -1L && sdonref <= t && (t <= sdoffref || sdoffref == -1L)) || (psdonref != -1L && psdonref <= t && t <= psdoffref))
...
static int tpann(WFDB_Time t) {
    ...
    if ((sdontest != -1L && sdontest <= t && (t <= sdofftest || sdofftest == -1L)) || (psdontest != -1L && psdontest <= t && t <= psdofftest))
...
static int verbose = 0;   /* if non-zero, describe all mismatches */
static long nrre = 0;     /* number of RR errors tallied in ssrre */
static double ssrre = 0.; /* sum of squares of RR errors */

/* count a beat label pair */
/* reference and test annotation types */
static void pair(int ref, int test) {
    switch (ref) {
    case 'N':
        switch (test) {
        case 'N':
...
/* map MIT annotation code into AAMI test label */
static int amap(int a) {
    switch (a) {
    case NORMAL:
    case LBBB:
...
static FILE *ofile, *sfile; /* files for beat-by-beat and shutdown reports */
...
static void pstat(char *s, char *f, long a, long b) {
...
static void sstat(char *s, char *f, long a, long b) {
    if (fflag == 1 || fflag == 3 || fflag == 4 || fflag == 6) {
...
static char *ofname = "-", *sfname; /* filenames for reports */
...
static void init(int argc, char *argv[]) {
    ...
    for (i = 1; i < argc; i++) {
        if (*argv[i] == '-') switch (*(argv[i] + 1)) {
            case 'a': /* annotator names follow */
                if (++i >= argc - 1) {
                    (void)fprintf(stderr, "%s: reference and test annotator names must follow -a\n", pname);
                    exit(1);
                }
                an[0].name = argv[i];
                an[1].name = argv[++i];
            ...
            case 'l': /* line-format output */
                if (++i >= argc - 1) {
                    (void)fprintf(stderr, "%s: two output file names must follow -l\n", pname);
                    exit(1);
                }
                ofname = argv[i];
                sfname = argv[++i];
            ...
            case 'w': /* match window follows */
                if (++i >= argc) {
                    (void)fprintf(stderr, "%s: match window must follow -w\n", pname);
                    exit(1);
                }
                match_dt = i;
                break;
            case 'x': /* custom output */
                CUSTOM = 1;
                break;
            default:
    ...
    if (start != 0L || end_time != 0L || match_dt != 0) (void)fprintf(stderr, "%s: (warning) nonstandard comparison selected\n", pname);
...
static void print_results(int fflag) {
    ...
    if (strcmp(ofname, "-")) {
        if ((ofile = fopen(ofname, "r")) == NULL) {
            ...
            if (fflag == 2) {
                if (CUSTOM) {
                    (void)fprintf(ofile, "Record\tNn'\tVn'\tFn'\tOn'\tNv\tVv\tFv'\tOv'\tNo'");
                    (void)fprintf(ofile, "\tVo'\tFo'\tQTP\tQFP\tQFN\tQER\tQSE\tQPP\tQF1\tVSE\tVPP\tVFPR\n");
                } else {
                    (void)fprintf(ofile, "Record Nn' Vn' Fn' On'  Nv   Vv  Fv' Ov' No'");
                    (void)fprintf(ofile, " Vo' Fo'  Q Se   Q +P   V Se   V +P  V FPR\n");
                }
            } else if (fflag == 5) {
                if (CUSTOM) {
                    (void)fprintf(ofile, "Record\tNn'\tSn'\tVn'\tFn'\tOn'\tNs\tSs\tVs\tFs'");
                    (void)fprintf(ofile, "\tOs'\tNv\tSv\tVv\tFv'\tOv'\tNo'\tSo'\tVo'\tFo'");
                    (void)fprintf(ofile, "\tQTP\tQFP\tQFN\tQER\tQSE\tQPP\tQF1\tVSE\tVPP\tSSE\tSPP\tRRerr\n");
                } else {
                    (void)fprintf(ofile, "Record Nn' Sn' Vn' Fn' On'  Ns  Ss  Vs  Fs'");
                    (void)fprintf(ofile, " Os' Nv  Sv   Vv  Fv' Ov' No' So' Vo' Fo'");
                    (void)fprintf(ofile, "  Q Se   Q +P   V Se   V +P   S Se   S +P RR err\n");
                }
            }
        } else {
            (void)fclose(ofile);
    ...
    if (fflag == 2 || fflag == 5) {
        if (strcmp(sfname, "-")) {
            if ((sfile = fopen(sfname, "r")) == NULL) {
                ...
                if (fflag == 2) {
                    (void)fprintf(sfile, "Record Nx   Vx   Fx   Qx  %% beats  %% N    ");
                    (void)fprintf(sfile, "%% V    %% F   Total Shutdown\n");
                    (void)fprintf(sfile, "                           missed missed ");
                    (void)fprintf(sfile, "missed missed      Time\n");
                } else {
                    if (CUSTOM) {
                        (void)fprintf(sfile, "Record\tNx\tSx\tVx\tFx\tQx\t%% beats\t%% N\t");
                        (void)fprintf(sfile, "%% S\t%% V\t%% F\tTotal Shutdown\n");
                    } else {
                        (void)fprintf(sfile, "Record Nx   Sx   Vx   Fx   Qx  %% beats  %% N    ");
                        (void)fprintf(sfile, "%% S    %% V    %% F   Total Shutdown\n");
                        (void)fprintf(sfile, "                                missed missed ");
    ...
    case 2: /* print line-format output */
        if (CUSTOM) {
            (void)fprintf(ofile, "%s\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld",
                          record,                                         //
            ...
            (void)fprintf(sfile, "%s\t%ld\t%ld\t%ld\t%ld", record, Nx + Sx, Vx, Fx, Qx);
        } else {
            (void)fprintf(ofile, "%4s %5ld %3ld %3ld %3ld %3ld %4ld %3ld %3ld %3ld %3ld %3ld",
                          record,                                         //
                          Nn + Ns + Nf + Nq + Sn + Ss + Sf + Sq,          //
                          Vn + Vs + Vf + Vq,                              //
                          Fn + Fs + Ff + Fq + Qn + Qs + Qf + Qq,          //
                          ...
                          Nv + Sv, Vv, Fv + Qv, Ov + Xv,                  //
                          No + Nx + So + Sx, Vo + Vx, Fo + Fx + Qo + Qx); //
            (void)fprintf(sfile, "%s %4ld %4ld %4ld %4ld  ", record, Nx + Sx, Vx, Fx, Qx);
        }
        break;
    ...
    if (CUSTOM && (fflag == 5 || fflag == 2)) {
        (void)fprintf(ofile, "\t%ld\t%ld\t%ld\t%ld", QTP, QFP, QFN, QFP + QFN);
    }
    ...
    if (CUSTOM && (fflag == 5 || fflag == 2)) {
        double se = (double)QTP / (double)(QTP + QFN);
        double pp = (double)QTP / (double)(QTP + QFP);
        double f1 = 2 * se * pp / (se + pp);
        (void)fprintf(ofile, "\t%.2f", f1 * 100.0);
    }
    ...
    if (fflag == 4 || fflag == 6) {
        ...
        if (nrre)
            (void)fprintf(ofile, "%6.2f ms", sqrt(ssrre / nrre) * 1000. / strtim("1"));
        else
            (void)fprintf(ofile, "     -");
    } else if (fflag == 5) {
        if (nrre)
    ...
    if (fflag >= 4) sstat("      S missed in shutdown", "%6.2f", Sx, Sn + Ss + Sv + Sf + Sq + So + Sx);
    sstat("      V missed in shutdown", "%6.2f", Vx, Vn + Vs + Vv + Vf + Vq + Vo + Vx);
    sstat("      F missed in shutdown", "%6.2f", Fx, Fn + Fs + Fv + Ff + Fq + Fo + Fx);
    if (fflag == 1 || fflag == 3 || fflag == 4 || fflag == 6) (void)fprintf(sfile, "       Total shutdown time: ");
    if (fflag != 2 && fflag != 5)
...
#ifdef MSDOS
    while (p >= s && *p != '\\' && *p != ':') {
        if (*p == '.') *p = '\0';                    /* strip off extension */
        if ('A' <= *p && *p <= 'Z') *p += 'a' - 'A'; /* convert to lower case */
        p--;
    }
#else
    while (p >= s && *p != '/')
...
static void genxcmp() {
    ...
    while ((end_time > 0L && (T <= end_time || t <= end_time)) ||  //
        ...
        if (t < T) {
            ...
            if (T - t <= match_dt && (T - t < abs(T - tprime) || aprime == 'O')) {
                ...
                if (test_annot.anntyp != ref_annot.anntyp || //
                    ...
                    if (test_annot.subtyp != ref_annot.subtyp || //
                        test_annot.chan != ref_annot.chan ||     //
                        test_annot.num != ref_annot.num) {       //
                        (void)sprintf(p, "[%d,%d,%d]", ref_annot.subtyp, ref_annot.chan, ref_annot.num);
                        p += strlen(p);
                    }
                    if (*ref_annot.aux) {
                    ...
                    p += strlen(p);
                    if (*test_annot.aux) (void)sprintf(p, " (%s)", test_annot.aux + 1);
                    alen = strlen(mstring + 1);
                    if (alen > 254) {
                        (void)fprintf(stderr,
                                      "aux string truncated at %s "
                                      "(%" WFDB_Pd_TIME ")\n",
                ...
                p = mstring + 1;
                (void)sprintf(p, "%c/%s", rpann(test_annot.time), annstr(test_annot.anntyp));
                p += strlen(p);
                if (*test_annot.aux) (void)sprintf(p, " (%s)", test_annot.aux + 1);
                alen = strlen(mstring + 1);
                if (alen > 254) {
...
#ifndef GOLIB
int main(int argc, char **argv) { return (bxb_main(argc, argv)); }
#endif
