...
static char *help_strings[] = {
    ...
    "include any of:", " -d          dump unfiltered and filtered samples on standard output;", "             do not annotate",
    " -f TIME     begin at specified time (default: beginning of the record)", " -h          print this usage summary",
    " -H          read multifrequency signals in high resolution mode", " -j          find and annotate J-points (QRS ends) as well as QRS onsets",
    " -m THRESH   set detector threshold to THRESH (default: 100)", /* TmDEF */
    " -p FREQ     specify power line (mains) frequency (default: 60)",
    /* PWFreqDEF */
    " -R          resample input at 120 or 150 Hz, depending on power line", "             frequency (default: do not resample)",
    " -s SIGNAL   analyze specified signal (default: 0)", " -t TIME     stop at specified time (default: end of the record)",
    " -v          verbose mode", "If too many beats are missed, decrease THRESH;  if there are too many extra", "detections, increase THRESH.", NULL};
...
int main(int argc, char **argv) {
    ...
    for (i = 1; i < argc; i++) {
        if (*argv[i] == '-') switch (*(argv[i] + 1)) {
            case 'd': /* dump filter data */ dflag = 1; break;
            ...
            case 'h': /* help requested */
                ...
                exit(0);
                break;
            case 'H': /* operate in WFDB_HIGHRES mode */ gvmode = WFDB_GVPAD | WFDB_HIGHRES; break;
            case 'j': /* annotate J-points (ends of QRS complexes) */ jflag = 1; break;
            case 'm': /* threshold */
            ...
            case 't': /* end time */
                ...
                to = i;
                break;
            case 'v': /* verbose mode */ vflag = 1; break;
    ...
    if (gvmode == 0 && (p = getenv("WFDBGVMODE"))) gvmode = atoi(p);
    ...
    if (sps < PWFreq) {
        (void)fprintf(stderr, "%s: sampling frequency (%g Hz) is too low%s", pname, sps, (gvmode & WFDB_HIGHRES) ? "\n" : ", try -H option\n");
        exit(3);
    }
    if (gvmode & WFDB_HIGHRES) setafreq(sampfreq((char *)NULL));
    a.name = "wqrs";
    a.stat = WFDB_WRITE;
    ...
    if ((t1 = strtim("8")) > BUFLN * 0.9) t1 = BUFLN / 2;
    t1 += from;
        ...
        if (ltsamp(t) > T1) { /* found a possible QRS near t */
            ...
            if (max > min + 10) { /* There is a QRS near tt */
                ...
                for (tt = t; tt > t - EyeClosing / 2; tt--) {
                    if (ltsamp(tt) - ltsamp(tt - 1) < onset && ltsamp(tt - 1) - ltsamp(tt - 2) < onset && ltsamp(tt - 2) - ltsamp(tt - 3) < onset &&
