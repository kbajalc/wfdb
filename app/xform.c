...
int main(int argc, char *argv[]) {
    char btstring[30], **description, **filename, *irec = NULL, *orec = NULL, *nrec = NULL, *startp = "0:0", **units;
    double *gain, ifreq, ofreq = 0.0;
    int clip = 0, *deltav, dflag = 0, fflag = 0, gflag = 0, Hflag = 0, i, iframelen, j, m, Mflag = 0, mn, *msiglist, n, nann = 0, nisig, nminutes = 0, nosig = 0, oframelen, reopen = 0, sflag = 0, *siglist = NULL, spf, uflag = 0, use_irec_desc = 1, *v, *vin, *vmax, *vmin, *vout, vt, *vv;
    ...
    if (nrec && orec && (strcmp(nrec, orec) == 0)) {
        (void)fprintf(stderr, "%s: arguments of -n (or -N) and -o cannot be identical\n", pname);
        exit(1);
    }
        ...
        if (sflag) {
            for (i = 0; i < nosig && nosig > 0; i++) {
                ...
                if ((siglist[i] = findsig(s)) < 0) {
                    (void)fprintf(stderr, "%s: warning: record %s doesn't have a signal '%s'\n", pname, irec, s);
    ...
    if ((vin = malloc(iframelen * sizeof(int))) == NULL || (vout = malloc(oframelen * sizeof(int))) == NULL) {
        (void)fprintf(stderr, "%s: insufficient memory\n", pname);
        exit(2);
    }
    ...
    if (orec != NULL) { /* an output record was specified */
        ...
        if (ifreq != ofreq) {
            if (Mflag != 0) {
                (void)fprintf(stderr, "%s: -M option may not be used if sampling frequency is to be changed\n", pname);
                wfdbquit();
                exit(1);
            }
        }
    ...
    /* If no output record was specified, get signal specs interactively. */
        ...
        if (nrec == NULL) {
            do {
                (void)fprintf(stderr, "Choose a name for the output record (up to %d characters): ", WFDB_MAXRNL);
        ...
        if (sflag == 0) {
            do {
                nosig = nisig;
                (void)fprintf(stderr, "Number of signals to be written (1-%d) [%d]: ", nisig, nosig);
                script_fgets(answer, sizeof(answer), ttyin);
                (void)sscanf(answer, "%d", &nosig);
                if (nosig == 0) {
                    (void)fprintf(stderr, "No signals will be written.  Are you sure? [n]: ");
                    script_fgets(answer, sizeof(answer), ttyin);
                    if (*answer != 'y' && *answer != 'Y') nosig = -1;
                }
            } while (nosig < 0 || nosig > nisig);
        }
        if (Mflag) {
            ofreq = ifreq;
            (void)fprintf(stderr, "Sampling frequency (%g frames/sec) will be unchanged\n", ofreq);
        } else
            do {
                ofreq = ifreq;
                (void)fprintf(stderr, "Output sampling frequency (Hz per signal, > 0) [%g]: ", ofreq);
                script_fgets(answer, sizeof(answer), ttyin);
                (void)sscanf(answer, "%lf", &ofreq);
            } while (ofreq < 0);
        if (ofreq == 0) ofreq = WFDB_DEFFREQ;
        if (nosig > 0) {
            (void)fprintf(stderr, "Specify the name of the directory in which the output\n");
            (void)fprintf(stderr, " signals should be written, or press <return> to write\n");
            ...
            (void)fprintf(stderr, "   16   16-bit two's complement amplitudes (LSB first)\n");
            (void)fprintf(stderr, "   61   16-bit two's complement amplitudes (MSB first)\n");
            (void)fprintf(stderr, "   80   8-bit offset binary amplitudes\n");
            (void)fprintf(stderr, "  160   16-bit offset binary amplitudes\n");
            (void)fprintf(stderr, "  212   2 12-bit amplitudes bit-packed in 3 bytes\n");
            (void)fprintf(stderr, "  310   3 10-bit amplitudes bit-packed in 15 LS bits of each of 2 words\n");
            ...
            fprintf(stderr, "  508  8-bit amplitudes with FLAC compression\n");
            fprintf(stderr, "  516  16-bit amplitudes with FLAC compression\n");
            fprintf(stderr, "  524  24-bit amplitudes with FLAC compression\n");
#endif
            do {
                format = dfin[0].fmt;
                (void)fprintf(stderr, "Choose an output format from the list above [%d]: ", format);
                script_fgets(answer, sizeof(answer), ttyin);
        ...
        for (i = 0; i < nosig; i++) {
            ...
            if (!use_irec_desc) {
                ...
                if ((description[i] = malloc(WFDB_MAXDSL + 2)) == NULL || (units[i] = malloc(WFDB_MAXUSL + 2)) == NULL) {
                    (void)fprintf(stderr, "%s: insufficient memory\n", pname);
                    exit(2);
                }
                (void)fprintf(stderr, "Signal %d description (up to %d characters): ", i, WFDB_MAXDSL);
                script_fgets(description[i], WFDB_MAXDSL + 2, ttyin);
                description[i][strlen(description[i]) - 1] = '\0';
                dfout[i].desc = description[i];
                (void)fprintf(stderr, "Signal %d units (up to %d characters): ", i, WFDB_MAXUSL);
            ...
            (void)fprintf(stderr, " Signal %d gain (adu/%s) [%g]: ", i, dfout[i].units ? dfout[i].units : "mV", dfout[i].gain);
            script_fgets(answer, sizeof(answer), ttyin);
            ...
            do {
                ...
                case 508:
                    ...
                    break;
                case 212:
                    if (dfout[i].adcres < 8 || dfout[i].adcres > 12) dfout[i].adcres = 12;
                    break;
                case 310:
                    if (dfout[i].adcres < 8 || dfout[i].adcres > 10) dfout[i].adcres = 10;
                    break;
                case 24:
                case 524:
                    if (dfout[i].adcres < 8 || dfout[i].adcres > 24) dfout[i].adcres = 24;
                    break;
                case 32:
                    if (dfout[i].adcres < 8 || dfout[i].adcres > 32) dfout[i].adcres = 32;
                    break;
                case 16:
                case 61:
                case 160:
                case 516:
                default:
                    if (dfout[i].adcres < 8 || dfout[i].adcres > 16) dfout[i].adcres = WFDB_DEFRES;
                    break;
                }
                (void)fprintf(stderr, " Signal %d ADC resolution in bits (8-32) [%d]: ", i, dfout[i].adcres);
                script_fgets(answer, sizeof(answer), ttyin);
                (void)sscanf(answer, "%d", &dfout[i].adcres);
            } while (dfout[i].adcres < 8 || dfout[i].adcres > 32);
            (void)fprintf(stderr, " Signal %d ADC zero level (adu) [%d]: ", i, dfout[i].adczero);
            script_fgets(answer, sizeof(answer), ttyin);
    ...
    if (to) {
        ...
        if (to > 0L && (nsamp = to - from) <= 0L) {
            (void)fprintf(stderr, "%s: 'to' time must be later than 'from' time\n", pname);
            exit(1);
        }
    }
    spm = strtim("1:0");
    ...
    if (nann > 0) {
        ...
        if (annopen(irec, ai, (unsigned)nann) < 0 || iannsettime(from) < -1) exit(2);
        ...
        for (i = 0; i < nann; i++) {
            tt = 0L;
            cc = -129;
            while (getann((unsigned)i, &annot) == 0 && (to == 0L || annot.time <= to)) {
                if (annot.time == 0 && annot.subtyp == 0 && annot.chan == 0 && annot.num == 0 && (annot.anntyp == NOTQRS || annot.anntyp == NOTE)) continue;
                ...
                /* If the -u option was specified, make sure that the corrected
                   ...
                   performed). */
                if (uflag && (annot.time < tt || (annot.time == tt && annot.chan <= cc)))
                    annot.time = ++tt;
                else
                    tt = annot.time;
                cc = annot.chan;
    ...
    for (i = 0; i < nosig; i++) {
        ...
        if (dfout[i].gain == 0.) {     /* output gain not specified */
            ...
            gain[i] = (double)(1L << dfout[i].adcres) / (double)(1L << dfin[j].adcres);
        ...
        if (dfout[i].gain != dfin[j].gain * gain[i] || dfout[i].baseline != dfin[j].baseline * gain[i]) {
    ...
    if (reopen) {
        ...
        for (i = 0; i < nosig; i++) {
            ...
            if ((p = (char *)malloc((unsigned)strlen(dfout[i].fname) + 1)) == (char *)NULL) {
            ...
            dfout[i].fname = p;
            if (dfout[i].units) {
                if ((p = (char *)malloc((unsigned)strlen(dfout[i].units) + 1)) == (char *)NULL) {
                    (void)fprintf(stderr, "%s: out of memory\n", pname);
                    exit(2);
                }
            ...
            if (dfout[i].desc) {
                if ((p = (char *)malloc((unsigned)strlen(dfout[i].desc) + 1)) == (char *)NULL) {
                    (void)fprintf(stderr, "%s: out of memory\n", pname);
                    exit(2);
                }
        ...
        if (osigfopen(dfout, (unsigned)nosig) != nosig) exit(2);
    }
    ...
    if (fflag == 0 && gflag == 0) { /* no frequency or gain changes */
        ...
        while (getvec(vin) >= nisig && (nsamp == -1L || nsamp-- > 0L)) {
            ...
            while (ot <= it) {
                ...
                for (i = 0; i < nosig; i++) {
                    if (v[i] == WFDB_INVALID_SAMPLE || vv[i] == WFDB_INVALID_SAMPLE)
    ...
    if (nrec) {
        ...
        if (xinfo = malloc(strlen(pname) + strlen(irec) + strlen(startp) + 80)) {
            (void)sprintf(xinfo, "Produced by %s from record %s, beginning at %s", pname, irec, startp);
            (void)putinfo(xinfo);
            for (i = 0; i < nosig; i++)
                if (siglist[i] != i) {
                    (void)sprintf(xinfo, "record %s, signal %d <- record %s, signal %d", nrec, i, irec, siglist[i]);
                    if (gain[i] != 1.) (void)sprintf(xinfo + strlen(xinfo), " * %g", gain[i]);
                    if (deltav[i] != 0) (void)sprintf(xinfo + strlen(xinfo), " %c %d", deltav[i] > 0 ? '+' : '-', deltav[i] > 0 ? deltav[i] : -deltav[i]);
                    (void)putinfo(xinfo);
                }
        }
        wfdbverbose();
...
static char *help_strings[] = {"usage: %s -i IREC [OPTIONS ...]\n",
