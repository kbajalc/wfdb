...
int main(int argc, char **argv) {
    ...
    for (; i < argc; i++) {
        if (*argv[i] == '-') switch (*(argv[i] + 1)) {
            ...
            case 'c': /* chan value follows */
                if (++i >= argc) {
                    (void)fprintf(stderr, "%s: chan value (between -128 and 127) must follow -c\n", pname);
                    exit(1);
                }
            ...
            case 'n': /* num value follows */
                if (++i >= argc) {
                    (void)fprintf(stderr, "%s: num value (between -128 and 127) must follow -n\n", pname);
                    exit(1);
                }
            ...
            case 'p': /* annotation mnemonic(s) follow */
                if (++i >= argc || !isann(j = strann(argv[i]))) {
                    (void)fprintf(stderr, "%s: annotation mnemonic(s) must follow -p\n", pname);
                    exit(1);
                }
                flag[j] = 1;
            ...
            case 's': /* subtyp value follows */
                if (++i >= argc) {
                    (void)fprintf(stderr, "%s: subtyp value (between -128 and 127) must follow -s\n", pname);
                    exit(1);
                }
    ...
    if ((sps = sampfreq(record)) < 0.) (void)setsampfreq(sps = WFDB_DEFFREQ);

    ai.stat = WFDB_READ;
    if (annopen(record, &ai, 1) < 0) /* open annotation file */
        exit(2);

    if ((tps = getiaorigfreq(0)) < sps) tps = sps;
    ...
    if (from) {
        if (*argv[(int)from] == 'a') {
            if ((afrom = atol(argv[(int)from] + 1)) < 0L) afrom = from = 0L;
            ...
            if ((bfrom = atol(argv[(int)from] + 1)) < 0L) bfrom = from = 0L;
