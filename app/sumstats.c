...
static counter Nn, Ns, Nv, No, Nx, Sn, Ss, Sv, So, Sx, Vn, Vs, Vv, Vo, Vx, Fn, Fs, Fv, Fo, Fx, Qx, On, Os, Ov;
...
static char s1[] = "Record Nn' Vn' Fn' On'  Nv   Vv  Fv' Ov' No' Vo' Fo'  Q Se   Q +P   V Se   V +P  V FPR\n";
static char s10[] = "Record\tNn'\tVn'\tFn'\tOn'\tNv\tVv\tFv'\tOv'\tNo'\tVo'\tFo'\tQTP\tQFP\tQFN\tQER\tQSE\tQPP\tQF1\tVSE\tVPP\tVFPR\n";
...
static char s40[] = "Record\tNn'\tSn'\tVn'\tFn'\tOn'\tNs\tSs\tVs\tFs'\tOs'\tNv\tSv\tVv\tFv'\tOv'\tNo'\tSo'\tVo'\tFo'\tQTP\tQFP\tQFN\tQER\tQSE\tQPP\tQF1\tVSE\tVPP\tSSE\tSPP\tRRerr\n";
// "Record	Nn'	Sn'	Vn'	Fn'	On'	Ns	Ss	Vs	Fs'	Os'	Nv	Sv	Vv	Fv'	Ov'	No'	So'	Vo'	Fo'	QSe	Q+P	VSe	V+P	SSe	S+P	RRerr\n";
static char s5[] = "Record Nx   Sx   Vx   Fx   Qx  % beats  % N    % S    % V    % F   Total Shutdown\n";
// static char s50[] = "Record\tNx\tSx\tVx\tFx\tQx\t% beats\t% N\t% S\t% V\t% F\tTotal Shutdown\n";
...
int sumstat_main(int argc, char *argv[]) {
    static char s[256];
    int type;
    FILE *ifile;
    ...
    if (argc < 2) {
        ...
        (void)fprintf(stderr, " FILE must be the name of an l-format file with column headings\n");
    ...
    if (strcmp(s, s1) == 0)
        type = 1; /* bxb -l beat-by-beat report */
    else if (strcmp(s, s10) == 0) {
        type = 1;
        CUSTOM = 1;
    } /* bxb -l beat-by-beat report */
    else if (strcmp(s, s2) == 0)
        type = 2; /* bxb -l shutdown report */
    else if (strcmp(s, s3) == 0)
        type = 3; /* rxr VE run-by-run report */
    else if (strcmp(s, s4) == 0)
        type = 4; /* bxb -L beat-by-beat report*/
    else if (strcmp(s, s40) == 0) {
        type = 4;
        CUSTOM = 1;
    } /* bxb -L beat-by-beat report*/
    else if (strcmp(s, s5) == 0)
        type = 5; /* bxb -L shutdown report */
    else if (strcmp(s, s6) == 0)
        type = 6; /* rxr SVE run-by-run report */
    else if (strcmp(s, s7a) == 0)
        type = 7; /* epic AF report */
    else if (strcmp(s, s7b) == 0)
        type = 7; /* epic VF report */
    else if (strcmp(s, s7c) == 0)
        type = 7; /* epic (2-signal) ST report */
    else if (strcmp(s, s7d) == 0)
        type = 7; /* epic ST signal 0 report */
    else if (strcmp(s, s7e) == 0)
        type = 7; /* epic ST signal 1 report */
    else if (strcmp(s, s8) == 0)
        type = 8; /* mxm report */
    else {
        (void)fprintf(stderr, "%s: input file %s does not appear to be a `bxb', `rxr', `mxm', or `epic'\n", pname, argv[1]);
    ...
    if (!CUSTOM) (void)printf("%s", s); /* print column headings */
    ...
    while (fgets(s, 256, ifile)) {
        ...
        if (unpack(type, s) == 0) {
            (void)fprintf(stderr, "%s: illegal format in %s:\n", pname, argv[1]);
    ...
    case 1: /* bxb -l beat-by-beat table */
        ...
        QFN = No + Vo + Fo;
        QFP = On + Ov;
        (void)printf("__________________________________________________________________");
    ...
    case 6: /* rxr SVE run-by-run table */
        ...
        (void)printf("Sum   %4" pctr " %3" pctr " %3" pctr " %3" pctr " %3" pctr " %3" pctr " %3" pctr " %3" pctr " %3" pctr " %3" pctr " %3" pctr " %3" pctr "\n", CTS, CFN, CTP, CFP, STS, SFN, STP, SFP, LTS, LFN, LTP, LFP);
        (void)printf("Gross                                                  ");
    ...
    case 4: /* bxb -L beat-by-beat table */
        ...
        if (CUSTOM) {
            ...
            (void)printf("QTP: %ld, QFP: %ld, QFN: %ld, QER: %ld :: QSE: %.2f, QPP: %.2f, QF1: %.2f\n", QTP, QFP, QFN, QFP + QFN, se * 100, pp * 100, f1 * 100);
        }
        break;
    ...
    case 7: /* epic report */
        (void)printf("______________________________________________________");
        (void)printf("__________________\n");
        (void)printf("Sum    %4" pctr " %4" pctr " %4" pctr " %4" pctr "                   %s", ETS, EFN, ETP, EFP, mstimstr(total_episode_length));
...
static int unpack(int type, char *s) {
    ...
    static char qtp[8], qfp[8], qfn[8], qer[8], qf1[8];
    ...
    case 1: /* bxb beat-by-beat report */
        fo = -1L;
        (void)sscanf(s, "%s%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "", rec, &nn, &vn, &fn, &on, &nv, &vv, &fv, &ov, &no, &vo, &fo);
        if (fo < 0L) return (0);
        Nn += nn;
        Vn += vn;
        Fn += fn;
    ...
    case 2: /* bxb -l shutdown report */
        st = -1L;
        (void)sscanf(s,
                     "%s%" sctr "%" sctr "%" sctr "%" sctr "%s%s%s%s%" sctr " seconds "
                     "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "",
                     rec, &nx, &vx, &fx, &qx, mb, mn, mv, mf, &st, &nt, &dummy, &vt, &ft, &qt);
        if (st < 0L) return (0);
        Nx += nx;
        Vx += vx;
        Fx += fx;
        ...
        QT += qt;
        if (nt + vt + ft + qt) {
            CBM += (nx + vx + fx + qx) / (double)(nt + vt + ft + qt);
            nrec++;
            if (nt) {
                CNM += nx / (double)nt;
                Nrec++;
    ...
    case 6:
        lfp = -1;
        (void)sscanf(s, "%s%d%d%d%d%d%d%d%d%d%d%d%d", rec, &cts, &cfn, &ctp, &cfp, &sts, &sfn, &stp, &sfp, &lts, &lfn, &ltp, &lfp);
        if (lfp < 0) return (0);
        CTS += cts;
        CFN += cfn;
        CTP += ctp;
    ...
    case 4: /* bxb -L beat-by-beat report */
        fo = -1L;
        if (CUSTOM) {
            (void)sscanf(s, "%s%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%s%s%s%s%s%s%s%s%s%s%s%s", rec, &nn, &sn, &vn, &fn, &on, &ns, &ss, &vs, &fs, &os, &nv, &sv, &vv, &fv, &ov, &no, &so, &vo, &fo, qtp, qfp, qfn, qer, qse, qpp, qf1, vse, vpp, sse, spp, srre);
        } else {
            (void)sscanf(s, "%s%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%s%s%s%s%s%s%s", rec, &nn, &sn, &vn, &fn, &on, &ns, &ss, &vs, &fs, &os, &nv, &sv, &vv, &fv, &ov, &no, &so, &vo, &fo, qse, qpp, vse, vpp, sse, spp, srre);
        }
        if (fo < 0L) return (0);
        Nn += nn;
        Sn += sn;
        ...
        if (nn + ns + nv + no + sn + ss + sv + so + vn + vs + vv + vo + fn + fs + fv + fo) {
            CQS += (nn + ns + nv + sn + ss + sv + vn + vs + vv + fn + fs + fv) / (double)(nn + ns + nv + no + sn + ss + sv + so + vn + vs + vv + vo + fn + fs + fv + fo);
            NQS++;
        }
        if (nn + sn + vn + fn + on + ns + ss + vs + fs + os + nv + sv + vv + fv + ov) {
            CQP += (nn + sn + vn + fn + ns + ss + vs + fs + nv + sv + vv + fv) / (double)(nn + sn + vn + fn + on + ns + ss + vs + fs + os + nv + sv + vv + fv + ov);
            NQP++;
        }
        if (vn + vs + vv + vo) {
    ...
    case 5: /* bxb -L shutdown report */
        st = -1L;
        (void)sscanf(s, "%s%" sctr "%" sctr "%" sctr "%" sctr "%" sctr "%s%s%s%s%s%" sctr "", rec, &nx, &sx, &vx, &fx, &qx, mb, mn, ms, mv, mf, &st);
        if (st < 0L) return (0);
        Nx += nx;
        Sx += sx;
        Vx += vx;
    ...
    case 7: /* epic report */
        tts[0] = '\0';
        (void)sscanf(s, "%s%" sctr "%" sctr "%" sctr "%" sctr "%s%s%s%s%s%s", rec, &ets, &efn, &etp, &efp, mb, mn, mds, mdp, rts, tts);
        if (tts[0] == '\0') return (0);
        ETS += ets;
        EFN += efn;
        ETP += etp;
...
#ifndef GOLIB
int main(int argc, char **argv) { return (sumstat_main(argc, argv)); }
#endif
