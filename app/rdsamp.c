...
int main(int argc, char *argv[]) {
    ...
    char *invalid, *snfmt, *tfmt, *tnfmt, *tufmt, *vfmt, speriod[16], tustr[21];
    int cflag = 0, highres = 0, i, isiglist, nsig, nosig = 0, pflag = 0, s, *sig = NULL, timeunits = SECONDS, vflag = 0, xflag = 0;

    int simple_flag = 0;

    WFDB_Frequency freq = 0, ofreq = 0;
    WFDB_Sample *v;
    WFDB_Siginfo *si;
    ...
    for (i = 1; i < argc; i++) {
        if (*argv[i] == '-') switch (*(argv[i] + 1)) {
            case 'x': simple_flag = 1; break;
            case 'z': // output frequency
                if (++i >= argc) {
                    (void)fprintf(stderr, "%s: frequency must follow -z\n", pname);
                    exit(1);
                }
                ofreq = atof(argv[i]);
                break;
            case 'c': /* output in CSV format */ cflag = 1; break;
            ...
            case 'h': /* help requested */
                ...
                exit(0);
                break;
            case 'H': /* select high-resolution mode */ highres = 1; break;
            case 'l': /* maximum length of output follows */
                ...
                maxl = i;
                break;
            case 'P': /* output in high-precision physical units */ ++pflag; /* (fall through to case 'p') */
            ...
            case 'S': /* search for valid sample of specified signal */
                if (++i >= argc) {
                    (void)fprintf(stderr, "%s: signal name or number must follow -S\n", pname);
                    exit(1);
                }
                search = argv[i];
                break;
            case 't': /* end time */
                ...
                to = i;
                break;
            case 'v': /* verbose output -- include column headings */ vflag = 1; break;
    ...
    if ((v = malloc(nsig * sizeof(WFDB_Sample))) == NULL || (si = malloc(nsig * sizeof(WFDB_Siginfo))) == NULL) {
    ...
    if (highres) setgvmode(WFDB_HIGHRES);
    ...
    if (freq != ofreq) {
        if (highres) {
            (void)fprintf(stderr, "%s: -H option may not be used if sampling frequency is to be changed\n", pname);
            wfdbquit();
            exit(1);
        }
        freq = ofreq;
    ...
    if (from > 0L && (from = strtim(argv[from])) < 0L) from = -from;
    if (isigsettime(from) < 0) exit(2);
    if (to > 0L && (to = strtim(argv[to])) < 0L) to = -to;
    ...
    if (search && ((s = findsig(search)) < 0 || (from = tnextvec(s, from)) < 0)) {
        (void)fprintf(stderr, "%s: can't read signal '%s'\n", pname, search);
        exit(2);
    }

    /* Reset 'to' if a duration limit was specified. */
    if (maxl > 0L && (maxl = strtim(argv[maxl])) < 0L) maxl = -maxl;
    if (maxl && (to == 0L || to > from + maxl)) to = from + maxl;
    ...
    if (cflag) { /* CSV output selected */
        ...
        if (pflag) { /* output in physical units */
            ...
            case TIMSTR:
                tnfmt = "'Time and date'";
                tufmt = "'hh:mm:ss dd/mm/yyyy'";
                break;
            case MSTIMSTR:
                tnfmt = "'Time and date'";
                tufmt = "'hh:mm:ss.mmm dd/mm/yyyy'";
                break;
            case HHMMSS:
                tnfmt = "'Elapsed time'";
                tufmt = "'hh:mm:ss.mmm'";
    ...
    if (pflag) {
        ...
        while ((to == 0L || from < to) && getvec(v) >= 0) {
            if (cflag == 0) {
                ...
                case HHMMSS: (void)printf("%15s", from == 0L ? "0:00.000" : mstimstr(from)); break;
...
static char *help_strings[] = {"usage: %s -r RECORD [OPTIONS ...]\n",
