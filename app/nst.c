...
int main(int argc, char *argv[]) {
    static char answer[20], buf[256], *wfdbp, irec[10], nrec[21], nnrec[20], orec[10], refaname[10], *protocol, tfname[20], *p, *s;
    ...
    for (i = 1; i < argc; i++) {
        if (*argv[i] == '-') switch (*(argv[i] + 1)) {
            case 'a': /* reference annotator name follows */
                if (i >= argc - 1) {
                    (void)fprintf(stderr, "%s: reference annotator name must follow -a\n", pname);
                    exit(1);
                }
            ...
            case 'F': /* format follows */
                if (i >= argc - 1) {
                    (void)fprintf(stderr, "%s: signal file format must follow -F\n", pname);
                    exit(1);
                }
                format = atoi(argv[++i]);
                break;
            ...
            case 'i': /* input record names follow */
                if (i >= argc - 2) {
                    (void)fprintf(stderr, "%s: input ECG and noise record names must follow -i\n", pname);
                    exit(1);
                }
            ...
            case 'p': /* protocol annotator name follows */
                if (i >= argc - 1) {
                    (void)fprintf(stderr, "%s: protocol annotator name must follow -p\n", pname);
                    exit(1);
                }
                protocol = argv[++i];
                break;
            case 's': /* signal-to-noise ratio follows */
                if (i >= argc - 1) {
                    (void)fprintf(stderr, "%s: signal-to-noise ratio (in dB) must follow -s\n", pname);
                    exit(1);
                }
                snr = atof(argv[++i]);
                break;
    ...
    wfdbp = getwfdb();
    if (*wfdbp != PSEP && !(*wfdbp == '.' && ((*wfdbp + 1) == PSEP || *(wfdbp + 1) == ' '))) {
        char *nwfdbp;
    ...
    if (refaname[0] == '\0') (void)strcpy(refaname, "atr");

    /* Get input record names if they were not specified with -i. */
    if (irec[0] == '\0' || nrec[0] == '\0') {
        (void)fprintf(stderr, "Enter the name of the existing ECG record (or `?' for help): ");
        ...
        while (annopen(irec, &ai, 1) < 0) {
            (void)fprintf(stderr, "Can't read annotator `%s' for record `%s'\n", ai.name, irec);
            (void)fprintf(stderr, "Enter the name of the reference annotator: ");
    ...
    if (protocol == (char *)NULL) {
        ...
        if (snr == -999999.0) {
            (void)fprintf(stderr, "Enter the desired signal-to-noise ratio in dB: ");
        ...
        (void)fprintf(stderr, "Checking RMS amplitude of record `%s' ...", nrec + 1);
        if (system(buf) != 0) exit(1);
        (void)fprintf(stderr, " done\n");
        if ((ifile = fopen(tfname, "r")) == NULL || fscanf(ifile, "%lf", &anoise[0]) < 1 || anoise[0] == 0.0) {
            (void)fprintf(stderr, "%s: can't read temporary file %s\n", pname, tfname);
            exit(1);
        }
        for (i = 1; fscanf(ifile, "%lf", &anoise[i]) == 1; i++) {
            if (anoise[i] == 0.0) {
                (void)fprintf(stderr, "%s: noise signal %d amplitude is zero -- can't normalize\n", pname, i);
                exit(2);
            }
        }
        (void)unlink(tfname);
        (void)sprintf(buf, "sigamp -a %s -r %s >%s", refaname, irec, tfname);
        (void)fprintf(stderr, "Checking normal QRS amplitude of record `%s' ...", irec);
        if (system(buf) != 0) exit(1);
        (void)fprintf(stderr, " done\n");
        if ((ifile = fopen(tfname, "r")) == NULL || fscanf(ifile, "%lf", &asig[0]) < 1 || asig[0] == 0.0) {
            (void)fprintf(stderr, "%s: can't read temporary file %s\n", pname, tfname);
            exit(1);
        }
        for (i = 1; fscanf(ifile, "%lf", &asig[i]) == 1; i++) {
            if (asig[i] == 0.0) {
                (void)fprintf(stderr, "%s: ECG signal %d amplitude is zero -- can't normalize\n", pname, i);
                exit(2);
            }
        }
        (void)fclose(ifile);
        ...
        for (i = 0; i < nisig; i++) {
            ...
            (void)fprintf(stderr, "[rec %s, sig %d] = [rec %s, sig %d] + %g * [rec %s, sig %d]\n", orec, i, irec, i, r[i], nrec + 1, i % nnsig);
        }
        ...
        if (sampfreq(irec) <= 0.) {
            ...
            if (sscanf(buf, "%s", answer) == 0) (void)strcpy(answer, "30:0");
        } else
        ...
        ai.stat = WFDB_WRITE;
        if (annopen(nrec + 1, &ai, 1) < 0) exit(3);
        annot.anntyp = NOTE;
        annot.aux = buf;
        for (t = t0; t < tf; t += dt) {
            buf[0] = '\0';
    ...
    if (0.9 * ssf > nsf || nsf > 1.1 * ssf) {
        (void)fprintf(stderr, "Sampling frequencies of records `%s' and `%s' differ significantly\n", irec, nrec + 1);
        p = nnrec;
        s = nrec + 1;
        while (*s != '\0' && *s != '_')
            *p++ = *s++;
        ...
        if (0.9 * ssf <= nsf && nsf <= 1.1 * ssf) {
            ...
            wfdbquit();
            (void)sprintf(buf, "xform -i %s -o %s -N %s -a %s", nrec + 1, tfname, nnrec, protocol);
            (void)fprintf(stderr, " ... resampling record `%s' ...", nrec + 1);
            if (system(buf) != 0) exit(1);
            (void)unlink(wfdbfile(protocol, nrec + 1));
            (void)unlink(wfdbfile("header", tfname));
            (void)fprintf(stderr, "Resampled noise record `%s' has been generated.\n", nnrec);
            (void)strcpy(nrec + 1, nnrec);
        }
    }
    wfdbverbose();
    ...
    (void)fprintf(stderr, "Copying reference annotations for record `%s' to `%s' ...", irec, orec);
    while (getann(0, &annot) == 0)
        (void)putann(0, &annot);
    wfdbquit();
    (void)fprintf(stderr, " done\n");
        ...
        while (t < annot.time && errct == 0) {
            ...
            if (--nlen <= 0L) {
                ...
                for (i = nse[0]; i < nse[0] + nnsig; i++)
                    if (si[i].group != si[i - 1].group && isgsettime(si[i].group, 0L) < 0) errct++;
                if (getvec(vin) < 0) errct++;
                /* Adjust offsets to avoid discontinuities. */
                for (i = 0; i < nisig; i++)
                    if (vin[i] != WFDB_INVALID_SAMPLE && vin[nse[i]] != WFDB_INVALID_SAMPLE && vout[i] != WFDB_INVALID_SAMPLE)
                        z[i] = vin[i] + gn[i] * vin[nse[i]] - vout[i];
            } else if (getvec(vin) < 0)
                errct++;
            for (i = 0; i < nisig; i++) {
                if (vin[i] != WFDB_INVALID_SAMPLE && vin[nse[i]] != WFDB_INVALID_SAMPLE && vout[i] != WFDB_INVALID_SAMPLE) {
...
cleanup:
    (void)newheader(orec);
    if (snr == -999999.0)
        (void)sprintf(buf, " Created by `%s' from records %s and %s, using protocol %s", pname, irec, nrec + 1, protocol);
    else
        (void)sprintf(buf, " Created by `%s' from records %s and %s (SNR = %g dB)", pname, irec, nrec + 1, snr);
    (void)putinfo(buf);
    wfdbquit();
}
...
static char *help_strings[] = {"usage: %s [OPTIONS ...]\n",
