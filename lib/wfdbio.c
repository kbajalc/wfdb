...
void wfdb_addtopath(const char *s) {
    ...
    for (p = s + strlen(s) - 1; p >= s && *p != '/' && *p != '\\' && *p != ':'; p--)
        ;
    ...
    for (c0 = c1 = wfdb_path_list, i = p - s; c1; c1 = c1->next) {
        if (strncmp(c1->prefix, s, i) == 0) {
            if (c0 == c1 || (c1->prev == c0 && strcmp(c0->prefix, ".") == 0)) return; /* no changes needed, quit */
...
FSTRING wfdberror(void) {
    if (!error_flag) wfdb_asprintf(&error_message, "WFDB library version %d.%d.%d (%s).\n", WFDB_MAJOR, WFDB_MINOR, WFDB_RELEASE, WFDB_BUILD_DATE);
    if (error_message)
...
FVOID wfdb_error(const char *format, ...) {
    ...
    if (error_print) MessageBox(GetFocus(), wfdberror(), "WFDB Library Error", MB_ICONASTERISK | MB_OK);
#endif
}
...
WFDB_FILE *wfdb_open(const char *s, const char *record, int mode) {
    ...
    for (c0 = wfdb_path_list; c0; c0 = c0->next) {
        ...
        */
        ...
        if (strcmp(wfdb_filename, long_filename) && (ifile = wfdb_fopen(wfdb_filename, RB)) != NULL) {
            wfdb_addtopath(wfdb_filename);
            SFREE(long_filename);
            SFREE(buf);
            SFREE(r);
...
static char *curl_get_ua_string(void) {
    ...
    /* The +3XX flag informs the server that this client understands
       ...
       enabled.) */
    wfdb_asprintf(&s, "libwfdb/%d.%d.%d (%s +3XX)", WFDB_MAJOR, WFDB_MINOR, WFDB_RELEASE, libcurl_ver);
    return (s);
}
...
static size_t curl_null_write(void *ptr, size_t size, size_t nmemb, void *stream) { return (size * nmemb); }
...
static const char *www_userpwd(const char *url) {
    ...
    for (i = 0; passwords && passwords[i]; i++) {
        ...
        n = p - passwords[i];
        if (strncmp(passwords[i], url, n) == 0 && (url[n] == 0 || url[n] == '/' || url[n - 1] == '/')) {
...
static void www_init(void) {
    if (!www_done_init) {
        ...
        if ((p = getenv("CURL_CA_BUNDLE")) && *p) curl_easy_setopt(curl_ua, CURLOPT_CAINFO, p);
        ...
        if ((p = getenv("WFDB_NET_DEBUG")) && *p) curl_easy_setopt(curl_ua, CURLOPT_VERBOSE, 1L);
...
static long www_get_cont_len(const char *url) {
    ...
    if (/* We just want the content length; NOBODY means we want to
       send a HEAD request rather than GET */
        ...
        || curl_try(curl_easy_setopt(curl_ua, CURLOPT_WRITEFUNCTION, curl_null_write)) ||
        curl_try(curl_easy_setopt(curl_ua, CURLOPT_HEADERFUNCTION, curl_null_write))
        /* Actually perform the request and wait for a response */
        || www_perform_request(curl_ua))
        return (0);

    if (curl_easy_getinfo(curl_ua, CURLINFO_CONTENT_LENGTH_DOWNLOAD, &length)) return (0);
...
static size_t curl_chunk_header_write(void *ptr, size_t size, size_t nmemb, void *stream) {
    char *s = (char *)ptr;
    ...
    if (0 == strncasecmp(s, "Content-Range:", 14)) {
        ...
        while (*s == ' ')
            s++;
        if (0 == strncasecmp(s, "bytes ", 6)) sscanf(s + 6, "%lu-%lu/%lu", &c->start_pos, &c->end_pos, &c->total_size);
    }
    return (size * nmemb);
}
...
static size_t curl_chunk_write(void *ptr, size_t size, size_t nmemb, void *stream) {
    size_t count = 0;
    char *p;
    struct chunk *c = (struct chunk *)stream;
...
static void curl_chunk_putb(struct chunk *chunk, char *data, size_t len) { curl_chunk_write(data, 1, len, chunk); }
...
static CHUNK *www_get_url_range_chunk(const char *url, long startb, long len) {
    ...
    if (url && *url) {
        ...
        if (/* In this case we want to send a GET request rather than
            ...
            || curl_try(curl_easy_setopt(curl_ua, CURLOPT_WRITEFUNCTION, curl_chunk_write))
            ...
            || curl_try(curl_easy_setopt(curl_ua, CURLOPT_HEADERFUNCTION, curl_chunk_header_write))
        ...
        if (!chunk->data) {
            chunk_delete(chunk);
            chunk = NULL;
        } else if (!curl_easy_getinfo(curl_ua, CURLINFO_EFFECTIVE_URL, &url2) && url2 && *url2 && strcmp(url, url2)) {
...
static netfile *nf_new(const char *url) {
    ...
    if (nf && url && *url) {
        ...
        if (chunk->start_pos == 0 && chunk->end_pos == chunk->size - 1 && chunk->total_size >= chunk->size && (chunk->size == page_size || chunk->size == chunk->total_size)) {
        ...
        if (nf->data == NULL) {
            if (chunk->size > 0L) wfdb_error("nf_new: insufficient memory (needed %ld bytes)\n", chunk->size);
...
static long nf_get_range(netfile *nf, long startb, long len, char *rbuf) {
    ...
    if (nf == NULL || nf->url == NULL || *nf->url == '\0' || startb < 0L || startb >= nf->cont_len || len <= 0L || rbuf == NULL) return (0L); /* invalid inputs -- fail silently */
    ...
    if (nf->mode == NF_CHUNK_MODE) { /* range requests acceptable */
        ...
        if (len <= page_size) { /* short request -- check if cached */
            if ((startb < nf->base_addr) || ((startb + len) > (nf->base_addr + page_size))) {
                ...
                if (chunk = nf_get_url_range_chunk(nf, startb, rlen)) {
                    ...
                    wfdb_error("nf_get_range: couldn't read %ld bytes of %s starting at %ld\n", len, nf->url, startb);
                    len = 0L;
                }
            }
...
static int nf_feof(netfile *nf) { return ((nf->err == NF_EOF_ERR) ? TRUE : FALSE); }

/* nf_eof returns true if the file pointer is at the EOF. */
static int nf_eof(netfile *nf) { return (nf->pos >= nf->cont_len) ? TRUE : FALSE; }
...
static int nf_ferror(netfile *nf) { return ((nf->err == NF_REAL_ERR) ? TRUE : FALSE); }
...
FINT LibMain(HINSTANCE hinst, WORD wDataSeg, WORD cbHeapSize, LPSTR lpszCmdLine) {
...
char FAR *wgetenv(char far *var) {
    ...
    while (*ep) {
        if (_fstrncmp(ep, var, l) == 0 && *(ep + l) == '=') /* Got it!  Skip '=', return a pointer to the value string. */
            return (ep + l + 1);
